...
const EMPTY_FORM: FormState = {
  name: "", lastName: "", roles: EMPTY_ROLES,
  availableDays: [5, 6], active: true,
};
  ...
  if (official.photoUrl) {
    return (
      <img
        ...
        alt={`${official.name} ${official.lastName}`}
        className={`${sz} rounded-full object-cover border border-white/10 flex-shrink-0`}
      />
    );
  }
  return (
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {ROLE_OPTIONS.filter((r) => roles[r.key]).map((r) => (
        <span key={r.key} className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${r.color}`}>
          {r.label}
        </span>
      ))}
    </div>
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {DAYS.filter((d) => days.includes(d.value)).map((d) => (
        <span key={d.value} className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-white/6 border border-white/10 text-white/50">
          {d.label}
        </span>
      ))}
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center px-4 pt-6 pb-28 overflow-y-auto">
      ...
      <div className="relative z-10 w-full max-w-lg glass-panel p-6 sm:p-8 rounded-3xl">
        ...
        <div className="space-y-5">
              ...
              {ROLE_OPTIONS.map((r) => (
                ...
                >
                  <div className={`w-4 h-4 rounded-md border flex items-center justify-center flex-shrink-0 transition-all ${
                    form.roles[r.key] ? "bg-current border-current" : "border-white/20"
                  }`}>
                    {form.roles[r.key] && (
          ...
          <div className="space-y-2">
            <label className="text-[11px] text-white/40 uppercase tracking-widest font-bold">
              Días disponibles <span className="text-red-400/60">*</span>
            </label>
            ...
            <div className="flex items-center justify-between glass-panel px-4 py-3 rounded-2xl">
              ...
              <button
                ...
                className={`w-11 h-6 rounded-full border transition-all relative ${
                  form.active ? "bg-brand-orange border-brand-orange" : "bg-white/8 border-white/15"
                }`}
              >
...
export default function OfficialsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FormState) => {
      const created = await apiPost<Official>("/api/officials", {
        name:          data.name.trim(),
    ...
    onSuccess: () => {
      ...
      closeModal();
      sileo.success({ title: "Árbitro registrado" });
    },
  ...
  const editMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: FormState }) => {
      const updated = await apiPut<Official>(`/api/officials/${id}`, {
        name:          data.name.trim(),
        lastName:      data.lastName.trim(),
        roles:         data.roles,
        availableDays: data.availableDays,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/officials"] });
      closeModal();
      sileo.success({ title: "Árbitro actualizado" });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/officials"] });
      sileo.success({ title: "Árbitro eliminado" });
    },
...
function OfficialCard({
  ...
  return (
    <div className="glass-panel rounded-2xl p-4 flex flex-col gap-3">
      ...
      <DayChips days={official.availableDays} />
